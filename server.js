...
const PIXORA_KINGDOM_TEMPLATE_VERSION=2;
...
function applyPixoraKingdomTemplateToSnapshot(snapshot){
  ...
  const gy=55,map=new Map(),backgrounds=[];
  ...
  // Destructive one-time clean rebuild requested for PIXORA. Recovery backup is written before this function runs.
  for(let x=0;x<100;x++)for(let y=gy;y<97;y++)set(x,y,'dirt');for(let y=97;y<100;y++)for(let x=0;x<100;x++)set(x,y,'bedrock');
  for(let x=7;x<=92;x++)set(x,gy,'royalBrick');for(let x=20;x<=79;x++)bg(x,gy-1,'royalWallpaper');
  fill(24,gy-12,34,gy-1,'royalStone');fill(65,gy-12,75,gy-1,'royalStone');for(let x=23;x<=35;x++)set(x,gy-13,'royalRoof');for(let x=64;x<=76;x++)set(x,gy-13,'royalRoof');for(const x of [24,27,31,34])set(x,gy-14,'royalGold');for(const x of [65,68,72,75])set(x,gy-14,'royalGold');for(const [x,y] of [[27,gy-9],[31,gy-6],[68,gy-9],[72,gy-6]])set(x,y,'royalGlass');
  fill(37,gy-11,62,gy-1,'royalBrick');fill(41,gy-16,58,gy-12,'royalStone');for(let x=40;x<=59;x++)set(x,gy-17,'royalRoof');for(const x of [40,43,47,50,54,58,59])set(x,gy-18,'royalGold');fillBg(38,gy-10,61,gy-2,'royalWallpaper');for(const [x,y] of [[42,gy-14],[46,gy-14],[53,gy-14],[57,gy-14],[40,gy-8],[59,gy-8]])bg(x,y,'royalWindowWallpaper');for(const [x,y] of [[44,gy-9],[55,gy-9],[28,gy-8],[71,gy-8]])bg(x,y,'royalBannerWallpaper');
  clearRect(47,gy-7,52,gy-1);fill(46,gy-7,46,gy-1,'royalStone');fill(53,gy-7,53,gy-1,'royalStone');for(let x=47;x<=52;x++)set(x,gy-8,'royalStone');set(46,gy-8,'royalGold');set(53,gy-8,'royalGold');fillBg(47,gy-7,52,gy-1,'royalBannerWallpaper');
  for(let y=gy-15;y<=gy-10;y++)set(48,y,'royalGold');for(let x=49;x<=52;x++)set(x,gy-15,'royalGold');set(52,gy-14,'royalGold');set(52,gy-13,'royalGold');for(let x=49;x<=51;x++)set(x,gy-12,'royalGold');for(let x=46;x<=53;x++)set(x,gy-20,'royalGold');for(const x of [46,49,53])set(x,gy-21,'royalRoof');set(45,gy-20,'royalRoof');set(54,gy-20,'royalRoof');
  for(let x=23;x<=35;x++)set(x,gy-7,'woodPlatform');for(let x=64;x<=76;x++)set(x,gy-7,'woodPlatform');tree(13);tree(86);tree(5);tree(94);for(const base of [9,80]){set(base,gy-2,'royalStone');set(base+7,gy-2,'royalStone');for(let x=base;x<=base+7;x++)set(x,gy-4,'royalRoof');bg(base+2,gy-2,'royalBannerWallpaper');bg(base+5,gy-2,'royalBannerWallpaper')}for(const x of [20,79]){set(x,gy-2,'royalStone');set(x,gy-1,'royalStone');set(x,gy-3,'royalGold')}
  clear(49,gy-1);clear(50,gy-1);clear(51,gy-1);set(50,gy,'bedrock');
  snapshot.blocks=[...map.values()];snapshot.backgrounds=backgrounds;snapshot.surfaceTile=gy;snapshot.door={x:50*40+2,y:gy*40-40,w:36,h:40};snapshot.templateVersion=PIXORA_KINGDOM_TEMPLATE_VERSION;snapshot.weather={type:'royal'};snapshot.caveBgRemoved=[];snapshot.plants=[];snapshot.locks=[];snapshot.placedDoors=[];snapshot.weatherOrbs=[];snapshot.jammers=[];snapshot.worldBans=[];snapshot.drops=[];snapshot.savedAt=Date.now();
  return true;
}
...
async function api(req,res){
  ...
  if(req.url==='/api/status')return send(res,200,{ok:true,build:'15.0.7',storage:storageMode,persistent:storageMode==='postgres'});
...
initStorage().then(()=>server.listen(PORT,()=>console.log(`Pixora Build 15.0.7 server running on port ${PORT}`))).catch(e=>{console.error(e);process.exit(1)});
