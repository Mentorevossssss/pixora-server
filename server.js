...
const PIXORA_KINGDOM_TEMPLATE_VERSION=3;
...
function applyPixoraKingdomTemplateToSnapshot(snapshot){
  ...
  const gy=55,map=new Map(),backgrounds=[],caveBgRemoved=[];
  ...
  // Build 15.0.8 destructive clean rebuild. Backup is created by ensurePixoraKingdom() first.
  // No Dirt, Rock, Cave Rock or generated natural underground remains.
  for(let y=gy+1;y<100;y++)for(let x=0;x<100;x++)caveBgRemoved.push(x+','+y);
  for(let x=0;x<100;x++)set(x,gy,'royalBrick');
  for(let x=0;x<100;x++)set(x,99,'bedrock');
  for(let x=19;x<=80;x++)bg(x,gy-1,'royalWallpaper');
  ...
  snapshot.blocks=[...map.values()];snapshot.backgrounds=backgrounds;snapshot.surfaceTile=gy;snapshot.door={x:50*40+2,y:(gy-1)*40,w:36,h:40};snapshot.templateVersion=PIXORA_KINGDOM_TEMPLATE_VERSION;snapshot.weather={type:'royal'};snapshot.caveBgRemoved=caveBgRemoved;snapshot.plants=[];snapshot.locks=[];snapshot.placedDoors=[];snapshot.weatherOrbs=[];snapshot.jammers=[];snapshot.worldBans=[];snapshot.drops=[];snapshot.savedAt=Date.now();
  return true;
}
...
const BLOCK_RARITY={grass:1,dirt:1,stone:1,caveStone:1,lava:1,wood:2,leaf:2,sand:2,brick:3,moss:3,glass:4,ice:3,metal:3,farmBlock:8,woodPlatform:3,woodDoor:5,worldDoor:10,royalStone:6,royalBrick:7,royalGold:10,royalGlass:8,royalRoof:8,bedrock:99};
...
const BLOCK_HITS_SERVER={leaf:2,sand:2,glass:2,dirt:3,grass:3,ice:3,moss:3,wood:4,stone:5,brick:5,lava:6,caveStone:6,metal:7,farmBlock:4,woodPlatform:3,woodDoor:4,worldDoor:6,royalStone:5,royalBrick:5,royalGold:6,royalGlass:3,royalRoof:4};
...
function applyWorldAction(state,action,session,world){
  ...
  if(type==='break'){
    ...
    state.snapshot.door={x:tx*40+2,y:ty*40,w:36,h:40};changed=true;result.door=state.snapshot.door;addWorldLog(dbNow,world,session.player,'main-door-move','Main Door',{tx,ty});saveDb(dbNow);
...
async function api(req,res){
  ...
  if(req.url==='/api/status')return send(res,200,{ok:true,build:'15.0.8',storage:storageMode,persistent:storageMode==='postgres'});
...
initStorage().then(()=>server.listen(PORT,()=>console.log(`Pixora Build 15.0.8 server running on port ${PORT}`))).catch(e=>{console.error(e);process.exit(1)});
