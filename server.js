...
function applyWorldAction(state,action,session,world){
  ...
  if(type==='break'){
    ...
    const i=state.snapshot.locks.findIndex(l=>Number(l.tx)===tx&&Number(l.ty)===ty);if(i<0)return {accepted:false};const l=state.snapshot.locks[i];if(String(l.owner||'')!==session?.player?.playerId)return {accepted:false};state.snapshot.locks.splice(i,1);addWorldLog(loadDb(),world,session.player,'lock-remove',l.type||'lock',{tx,ty});changed=true;
