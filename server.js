...
function worldLockFor(world){const state=worldStates.get(world)||loadWorldFromDb(world);return state?.snapshot?.locks?.find(l=>l?.type==='worldLock')||null}
...
function runChatCommand({raw,world,session,db}){
  ...
  if(cmd==='worldinfo'){const wl=worldLockFor(world),owner=wl?knownPlayerLabel(db,wl.owner):'None',admins=wl&&Array.isArray(wl.trusted)?wl.trusted.map(id=>knownPlayerLabel(db,id)).filter(Boolean):[],slow=Math.max(0,Number(db.worldSlowmode?.[world]||0)),banned=!!db.bannedWorlds?.[world];return ok(`WORLD ${world} • Owner: ${owner} • World Admin: ${admins.join(', ')||'None'} • Players: ${room?.size||0}/${MAX_WORLD_PLAYERS} • Slowmode: ${slow}s • World Ban: ${banned?'YES':'NO'}`,{history:true,historyName:'WORLD INFO'});}
  if(cmd==='msg'||cmd==='r'){
  ...
  if(cmd==='give'){if(!hasRole(role,'Admin'))return commandError();const item=String(parts[0]||'').slice(0,40),amount=Math.max(1,Math.min(200,Math.floor(Number(parts[1]||1))));let target=session.player;if(parts[2]){const r=resolveWorldTarget(world,parts[2]);if(!r.player)return commandError('Target player tidak ditemukan.');if(role!=='Owner'&&!canStaffTarget(session.player,r.player)&&r.player.playerId!==pid)return commandError('Target role terlalu tinggi.');target=r.player}if(!item)return commandError('Pakai: /give <item> [amount] [player]');if(item.toLowerCase()==='bedrock'&&(role!=='Owner'||String(target.playerId)!==String(pid)))return commandError('BEDROCK hanya dapat diberikan Owner ke dirinya sendiri.');const targetSave=getPlayerSave(db,target.playerId);if(!targetSave)return commandError('Target belum memiliki save server.');const inv=targetSave.inventory||{};inv[item]=Math.min(200,Number(inv[item]||0)+amount);targetSave.inventory=inv;setPlayerSave(db,target.playerId,targetSave);saveDb(db);const live=sessionByPlayerId(target.playerId)?.session;if(live)queueEvent(target.playerId,{type:'staff-save-refresh',playerSave:targetSave});addAudit(db,session.player,'give',target.playerId,{item,amount});saveDb(db);return ok('Gave '+amount+' '+item+' to '+target.displayName)}
...
function serverLockBounds(l){
  const sizes={smallLock:10,bigLock:20,hugeLock:35,worldLock:100},size=sizes[String(l?.type||'worldLock')]||100;if(size>=100)return {x1:0,y1:0,x2:99,y2:99};const half=Math.floor(size/2);return {x1:Number(l.tx)-half,y1:Number(l.ty)-half,x2:Number(l.tx)-half+size-1,y2:Number(l.ty)-half+size-1}
}
...
function applyWorldAction(state,action,session,world){
  ...
  if(type==='break'){
    ...
    state.snapshot.locks.push({tx,ty,type:lt,owner:session?.player?.playerId||'',trusted:[],hitsTaken:0});addWorldLog(loadDb(),world,session.player,'lock-place',lt,{tx,ty});changed=true;
  }else if(type==='lock-remove'){
    const i=state.snapshot.locks.findIndex(l=>Number(l.tx)===tx&&Number(l.ty)===ty);if(i<0)return {accepted:false};const l=state.snapshot.locks[i];if(String(l.owner||'')!==session?.player?.playerId&&!ownerBreakActive(session))return {accepted:false};state.snapshot.locks.splice(i,1);addWorldLog(loadDb(),world,session.player,'lock-remove',l.type||'lock',{tx,ty});changed=true;
...
async function api(req,res){
  ...
  if(req.url==='/api/status')return send(res,200,{ok:true,build:'15.0.4',storage:storageMode,persistent:storageMode==='postgres'});
...
initStorage().then(()=>server.listen(PORT,()=>console.log(`Pixora Build 15.0.4 server running on port ${PORT}`))).catch(e=>{console.error(e);process.exit(1)});
