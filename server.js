...
const PIXORA_KINGDOM_TEMPLATE_VERSION=4;
...
function sanitizeWorldSnapshot(input){
  ...
  const ROYAL_BACKGROUNDS=new Set(['royalWallpaper','royalBannerWallpaper','royalWindowWallpaper']);
  const backgrounds=Array.isArray(s.backgrounds)?s.backgrounds.slice(0,12000).filter(bg=>bg&&Number.isFinite(Number(bg.tx))&&Number.isFinite(Number(bg.ty))&&ROYAL_BACKGROUNDS.has(String(bg.t||''))).map(bg=>({tx:Math.floor(Number(bg.tx)),ty:Math.floor(Number(bg.ty)),t:String(bg.t)})):[];
...
function applyPixoraKingdomTemplateToSnapshot(snapshot){
  ...
  const gy=82,map=new Map(),backgrounds=[],caveBgRemoved=[];
  const set=(tx,ty,t)=>{if(tx<0||ty<0||tx>=100||ty>=100)return;map.set(tx+','+ty,{x:tx*40,y:ty*40,t})};
  const clear=(tx,ty)=>map.delete(tx+','+ty),fill=(x1,y1,x2,y2,t)=>{for(let y=y1;y<=y2;y++)for(let x=x1;x<=x2;x++)set(x,y,t)},clearRect=(x1,y1,x2,y2)=>{for(let y=y1;y<=y2;y++)for(let x=x1;x<=x2;x++)clear(x,y)};
  const bg=(tx,ty,t='royalWallpaper')=>{const i=backgrounds.findIndex(o=>o.tx===tx&&o.ty===ty);if(i>=0)backgrounds.splice(i,1);backgrounds.push({tx,ty,t})},fillBg=(x1,y1,x2,y2,t='royalWallpaper')=>{for(let y=y1;y<=y2;y++)for(let x=x1;x<=x2;x++)bg(x,y,t)};
  ...
  // Build 15.0.9 destructive clean rebuild; ensurePixoraKingdom writes a recovery backup before this runs.
  for(let y=gy+1;y<100;y++)for(let x=0;x<100;x++)caveBgRemoved.push(x+','+y);
  // Full lower-world royal foundation: no natural terrain and no giant empty void below the kingdom.
  for(let y=gy;y<99;y++)for(let x=0;x<100;x++)set(x,y,((y-gy)%4===0)?'royalBrick':'royalStone');
  for(let y=gy+2;y<99;y+=5)for(let x=0;x<100;x+=10)set(x,y,'royalGold');
  for(let x=0;x<100;x++)set(x,99,'bedrock');
  for(let x=19;x<=81;x++)bg(x,gy-1,'royalWallpaper');

  fill(24,gy-12,34,gy-1,'royalStone');fill(66,gy-12,76,gy-1,'royalStone');for(let x=23;x<=35;x++)set(x,gy-13,'royalRoof');for(let x=65;x<=77;x++)set(x,gy-13,'royalRoof');for(const x of [24,27,31,34])set(x,gy-14,'royalGold');for(const x of [66,69,73,76])set(x,gy-14,'royalGold');for(const [x,y] of [[27,gy-9],[31,gy-6],[69,gy-9],[73,gy-6]])set(x,y,'royalGlass');
  fill(37,gy-11,63,gy-1,'royalBrick');fill(41,gy-16,59,gy-12,'royalStone');for(let x=40;x<=60;x++)set(x,gy-17,'royalRoof');for(const x of [40,43,47,50,54,58,60])set(x,gy-18,'royalGold');fillBg(38,gy-10,62,gy-2,'royalWallpaper');for(const [x,y] of [[42,gy-14],[46,gy-14],[54,gy-14],[58,gy-14],[40,gy-8],[60,gy-8]])bg(x,y,'royalWindowWallpaper');for(const [x,y] of [[44,gy-9],[56,gy-9],[28,gy-8],[72,gy-8]])bg(x,y,'royalBannerWallpaper');
  clearRect(48,gy-7,52,gy-1);fill(47,gy-7,47,gy-1,'royalStone');fill(53,gy-7,53,gy-1,'royalStone');for(let x=48;x<=52;x++)set(x,gy-8,'royalStone');set(47,gy-8,'royalGold');set(53,gy-8,'royalGold');fillBg(48,gy-7,52,gy-1,'royalBannerWallpaper');
  for(let y=gy-15;y<=gy-10;y++)set(48,y,'royalGold');for(let x=49;x<=52;x++)set(x,gy-15,'royalGold');set(52,gy-14,'royalGold');set(52,gy-13,'royalGold');for(let x=49;x<=51;x++)set(x,gy-12,'royalGold');for(let x=46;x<=54;x++)set(x,gy-20,'royalGold');for(const x of [46,50,54])set(x,gy-21,'royalRoof');set(45,gy-20,'royalRoof');set(55,gy-20,'royalRoof');
  for(let x=23;x<=35;x++)set(x,gy-7,'woodPlatform');for(let x=65;x<=77;x++)set(x,gy-7,'woodPlatform');tree(13);tree(87);tree(5);tree(95);for(const base of [9,83]){set(base,gy-2,'royalStone');set(base+7,gy-2,'royalStone');for(let x=base;x<=base+7;x++)set(x,gy-4,'royalRoof');bg(base+2,gy-2,'royalBannerWallpaper');bg(base+5,gy-2,'royalBannerWallpaper')}for(const x of [20,80]){set(x,gy-2,'royalStone');set(x,gy-1,'royalStone');set(x,gy-3,'royalGold')}

  snapshot.blocks=[...map.values()];snapshot.backgrounds=backgrounds;snapshot.surfaceTile=gy;snapshot.door={x:50*40+2,y:(gy-1)*40,w:36,h:40};snapshot.templateVersion=PIXORA_KINGDOM_TEMPLATE_VERSION;snapshot.weather={type:'royal'};snapshot.caveBgRemoved=caveBgRemoved;snapshot.plants=[];snapshot.locks=[];
  snapshot.placedDoors=[{id:'PIXORA-ROYAL-GATE',tx:50,ty:gy-1,type:'royalGateDoor',owner:'',doorId:'',targetWorld:'',targetId:'',open:true,systemGate:true}];
  snapshot.weatherOrbs=[];snapshot.jammers=[];snapshot.worldBans=[];snapshot.drops=[];snapshot.savedAt=Date.now();
  return true;
}
...
function worldActivityScore(s){if(!s)return 0;return (s.locks?.length||0)*50+(s.plants?.length||0)*14+(s.placedDoors?.length||0)*22+(s.backgrounds?.length||0)*3+(s.weatherOrbs?.length||0)*10+(s.jammers?.length||0)*10+(s.caveBgRemoved?.length||0)*2+(s.worldBans?.length||0)*10}
...
const BLOCK_RARITY={grass:1,dirt:1,stone:1,caveStone:1,lava:1,wood:2,leaf:2,sand:2,brick:3,moss:3,glass:4,ice:3,metal:3,farmBlock:8,woodPlatform:3,woodDoor:5,worldDoor:10,royalGateDoor:12,royalStone:6,royalBrick:7,royalGold:10,royalGlass:8,royalRoof:8,bedrock:99};
...
const BLOCK_HITS_SERVER={leaf:2,sand:2,glass:2,dirt:3,grass:3,ice:3,moss:3,wood:4,stone:5,brick:5,lava:6,caveStone:6,metal:7,farmBlock:4,woodPlatform:3,woodDoor:4,worldDoor:6,royalGateDoor:7,royalStone:5,royalBrick:5,royalGold:6,royalGlass:3,royalRoof:4};
...
function applyWorldAction(state,action,session,world){
  ...
  if(type==='owner-break'){
    ...
    let i=state.snapshot.placedDoors.findIndex(d=>Number(d.tx)===tx&&Number(d.ty)===ty);if(i>=0){result.removedKind='door';result.removedType=state.snapshot.placedDoors[i].type;state.snapshot.placedDoors.splice(i,1);changed=true}
    else{const main=state.snapshot.door;if(main&&px<Number(main.x||0)+Number(main.w||36)&&px+40>Number(main.x||0)&&py<Number(main.y||0)+Number(main.h||40)&&py+40>Number(main.y||0)){state.snapshot.door=null;changed=true;result.removedKind='main-door'}
      ...
      else if((i=(state.snapshot.backgrounds||[]).findIndex(bg=>Number(bg.tx)===tx&&Number(bg.ty)===ty))>=0){result.removedKind='background';result.removedType=state.snapshot.backgrounds[i].t;state.snapshot.backgrounds.splice(i,1);changed=true}
      else{const bgKey=tx+','+ty;if(!state.snapshot.caveBgRemoved.includes(bgKey)){state.snapshot.caveBgRemoved.push(bgKey);changed=true;result.removedKind='cave-background'}}
    }
  ...
  }else if(type==='background-place'){
    if(!canSessionEditTile(state,session,tx,ty))return {accepted:false};const bt=String(action.backgroundType||'').slice(0,40),allowed=new Set(['royalWallpaper','royalBannerWallpaper','royalWindowWallpaper']);if(!allowed.has(bt))return {accepted:false};state.snapshot.backgrounds=Array.isArray(state.snapshot.backgrounds)?state.snapshot.backgrounds:[];if(state.snapshot.backgrounds.some(bg=>Number(bg.tx)===tx&&Number(bg.ty)===ty))return {accepted:false};state.snapshot.backgrounds.push({tx,ty,t:bt});changed=true;
  }else if(type==='background-break'){
    if(!canSessionEditTile(state,session,tx,ty))return {accepted:false};if(state.snapshot.blocks.some(key)||state.snapshot.plants.some(p=>Number(p.tx)===tx&&Number(p.ty)===ty)||state.snapshot.locks.some(l=>Number(l.tx)===tx&&Number(l.ty)===ty)||state.snapshot.placedDoors.some(d=>Number(d.tx)===tx&&Number(d.ty)===ty)||state.snapshot.weatherOrbs.some(o=>Number(o.tx)===tx&&Number(o.ty)===ty)||state.snapshot.jammers.some(j=>Number(j.tx)===tx&&Number(j.ty)===ty))return {accepted:false};state.snapshot.backgrounds=Array.isArray(state.snapshot.backgrounds)?state.snapshot.backgrounds:[];const i=state.snapshot.backgrounds.findIndex(bg=>Number(bg.tx)===tx&&Number(bg.ty)===ty);if(i<0)return {accepted:false};const [removed]=state.snapshot.backgrounds.splice(i,1);dropsAdded=addDrops(state,[makeDrop(tx,ty,removed.t,1,'item',200,12)]);changed=true;
  ...
  }else if(type==='main-door-move'){
    const dbNow=loadDb(),save=getPlayerSave(dbNow,session?.player?.playerId),hasMover=String(session?.player?.role||'')==='Owner'||Number(save?.inventory?.doorMover||0)>0;
    if(!hasMover||!canSessionEditTile(state,session,tx,ty)||!sessionTileInReach(world,session,tx,ty))return {accepted:false};
    const belowKey=(b)=>Math.floor(Number(b.x)/40)===tx&&Math.floor(Number(b.y)/40)===ty+1;
    const floorBlock=state.snapshot.blocks.find(belowKey),solidFloor=!!floorBlock&&!['wood','leaf','woodPlatform'].includes(String(floorBlock.t||''));
    const occupied=state.snapshot.blocks.some(key)||state.snapshot.plants.some(p=>Number(p.tx)===tx&&Number(p.ty)===ty)||state.snapshot.locks.some(l=>Number(l.tx)===tx&&Number(l.ty)===ty)||state.snapshot.placedDoors.some(d=>Number(d.tx)===tx&&Number(d.ty)===ty)||state.snapshot.weatherOrbs.some(o=>Number(o.tx)===tx&&Number(o.ty)===ty)||state.snapshot.jammers.some(j=>Number(j.tx)===tx&&Number(j.ty)===ty)||tileOccupiedByPlayer(world,tx,ty);
    if(occupied||ty>=99||!solidFloor)return {accepted:false};
    const systemGate=(state.snapshot.placedDoors||[]).find(d=>d.systemGate&&d.type==='royalGateDoor');if(systemGate){systemGate.tx=tx;systemGate.ty=ty}state.snapshot.door={x:tx*40+2,y:ty*40,w:36,h:40};changed=true;result.door=state.snapshot.door;result.movedSystemGate=!!systemGate;addWorldLog(dbNow,world,session.player,'main-door-move','Main Door',{tx,ty,royalGateMoved:!!systemGate});saveDb(dbNow);
  }else if(type==='door-place'){
    if(!canSessionEditTile(state,session,tx,ty))return {accepted:false};
    const dt=String(action.doorType||'');if(!['woodDoor','worldDoor','royalGateDoor'].includes(dt))return {accepted:false};
    if(tileOccupiedByPlayer(world,tx,ty)||state.snapshot.blocks.some(key)||state.snapshot.jammers.some(j=>Number(j.tx)===tx&&Number(j.ty)===ty)||state.snapshot.plants.some(p=>Number(p.tx)===tx&&Number(p.ty)===ty)||state.snapshot.locks.some(l=>Number(l.tx)===tx&&Number(l.ty)===ty)||state.snapshot.weatherOrbs.some(o=>Number(o.tx)===tx&&Number(o.ty)===ty)||state.snapshot.placedDoors.some(d=>Number(d.tx)===tx&&Number(d.ty)===ty))return {accepted:false};
    const d={id:randomId('DOOR'),tx,ty,type:dt,owner:session?.player?.playerId||'',doorId:'',targetWorld:'',targetId:'',open:false,systemGate:false};state.snapshot.placedDoors.push(d);changed=true;result.door=d;
  }else if(type==='door-update'){
    const i=state.snapshot.placedDoors.findIndex(d=>Number(d.tx)===tx&&Number(d.ty)===ty);if(i<0)return {accepted:false};const d=state.snapshot.placedDoors[i];const systemRoyalGate=!!d.systemGate&&d.type==='royalGateDoor';if(systemRoyalGate?String(session?.player?.role||'')!=='Owner':String(d.owner||'')!==session?.player?.playerId)return {accepted:false};if(d.type==='worldDoor'){d.doorId=String(action.doorId||'').replace(/[^A-Za-z0-9_-]/g,'').slice(0,18);d.targetWorld=cleanWorld(action.targetWorld);d.targetId=String(action.targetId||'').replace(/[^A-Za-z0-9_-]/g,'').slice(0,18)}else d.open=!!action.open;changed=true;result.door=d;
  }else if(type==='door-remove'){
    const i=state.snapshot.placedDoors.findIndex(d=>Number(d.tx)===tx&&Number(d.ty)===ty);if(i<0)return {accepted:false};const d=state.snapshot.placedDoors[i];if(d.systemGate){if(!ownerBreakActive(session))return {accepted:false}}else if(String(d.owner||'')!==session?.player?.playerId&&!ownerBreakActive(session))return {accepted:false};state.snapshot.placedDoors.splice(i,1);changed=true;result.removedDoor=d;
...
async function api(req,res){
  ...
  if(req.url==='/api/status')return send(res,200,{ok:true,build:'15.0.9',storage:storageMode,persistent:storageMode==='postgres'});
  ...
  if(req.url==='/api/register'){
    ...
    const player={playerId:account.playerId,displayName:rawId,accountId:id,accountType:'account',role:roleForAccount(account)},t=createSession(player);return send(res,200,{token:t,player:playerPayload(player),playerSave:null,testGrants:player.role==='Owner'?['alphaWing','pixoraHat','doorMover']:[]})
  }
  if(req.url==='/api/login'){
    ...
    const player={playerId:account.playerId,displayName:account.username,accountId:id,accountType:'account',role:roleForAccount(account)},t=createSession(player);return send(res,200,{token:t,player:playerPayload(player),playerSave:account.playerSave||null,testGrants:roleForAccount(account)==='Owner'?['alphaWing','pixoraHat','doorMover']:[]})
  }
...
initStorage().then(()=>server.listen(PORT,()=>console.log(`Pixora Build 15.0.9 server running on port ${PORT}`))).catch(e=>{console.error(e);process.exit(1)});
